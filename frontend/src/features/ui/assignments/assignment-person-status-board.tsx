...
import { AssignmentOrderCard } from "@/features/ui/assignments/assignment-order-card";
          ...
          <div className={cn("min-w-0", isRtl && "text-right")}>
            {!isUnassigned ? (
              <StaffNameWithBadges
                ...
                t={t}
                isRtl={isRtl}
                nameClassName="font-display text-lg font-bold text-foreground"
              />
            ) : (
            ...
            <p className="text-sm text-muted-slate">
              {t.assignments.personBoardSuitSubtitle.replace(
                "{count}",
                String(person.suitCount),
              )}
            </p>
          </div>
        </div>
