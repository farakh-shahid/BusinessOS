...
import { boardColumnBorderClass } from "@/features/infrastructure/data/order-list-ui";
...
function RoleSuitPill({
  ...
  return (
    <span className="inline-flex items-center gap-1 rounded-full border border-hairline bg-background px-2 py-0.5 text-[10px] font-semibold text-muted-slate">
      <span>{label}</span>
    ...
    >
      ...
      >
        ...
        <span className="shrink-0 rounded-full bg-background px-2.5 py-1 text-[11px] font-semibold text-muted-slate ring-1 ring-hairline">
          {person.suitCount} {t.assignments.activeSuits}
        </span>
      </div>
      ...
      >
        <RoleSuitPill label={t.assignments.roleBooked} count={person.bookedSuits} />
        <RoleSuitPill label={t.orderStatus.cutting} count={person.cutSuits} />
        <RoleSuitPill
          label={t.orderStatus.stitching}
          count={person.stitchedSuits}
        />
