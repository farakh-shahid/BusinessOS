...
/** Short label for tight list steppers, e.g. "Hassan A." */
...
export type ProductionStageKey = "cutting" | "stitching";

export function productionStageAssignees(
  order: Pick<
    Order,
    "workflowStatus" | "assignedToName" | "cuttingMasterName" | "stitchingMasterName"
  >,
): Partial<Record<ProductionStageKey, string>> {
  ...
  const result: Partial<Record<ProductionStageKey, string>> = {};

  if (cutting) result.cutting = cutting;
  if (stitching) result.stitching = stitching;

  if (!result.stitching && worker) {
    if (
      order.workflowStatus === "stitching" ||
      order.workflowStatus === "ready"
    ) {
      result.stitching = worker;
    }
  }

  if (!result.cutting && worker) {
    if (
      order.workflowStatus === "pending" ||
      order.workflowStatus === "cutting"
    ) {
      result.cutting = worker;
    }
  }
...
export function currentStageWorkerLine(
  order: Pick<
    Order,
    | "workflowStatus"
    | "assignedToName"
    ...
    | "stitchingMasterName"
  >,
): { stage: "cutting" | "stitching"; name: string | null } | null {
  if (
    order.workflowStatus === "cancelled" ||
    order.workflowStatus === "delivered"
  ) {
  ...
  const assignees = productionStageAssignees(order);

  if (
    order.workflowStatus === "pending" ||
    order.workflowStatus === "cutting"
  ) {
    return { stage: "cutting", name: assignees.cutting ?? null };
  }
  ...
  if (
    ...
    return { stage: "stitching", name: assignees.stitching ?? null };
  }
