...
export class OrderService {
  ...
  getProductionPerformance(
    ...
    return this.orders.getProductionPerformance(tenantId, from, to, worker);
  }
  ...
  async getFullById(
    ...
    const [auditLog, payments, customerLastStitchingMasterName] = await Promise.all([
      this.audit.listForOrder(tenantId, orderId),
      this.orders.getPaymentsWithUsers(tenantId, orderId),
      this.orders.findLastStitchingMasterForCustomer(
        tenantId,
        order.customerId,
        orderId,
      ),
    ...
    const withHistory = {
      ...detail,
      customerLastStitchingMasterName,
    };
    return user && shouldHideFinancials(user.role)
      ? sanitizeOrderFullDetailForStaff(withHistory)
      : withHistory;
  }
  ...
  async markReady(
    ...
    if (isStaffRole(user.role)) {
      throw new ForbiddenException("Staff cannot change order status");
    }
