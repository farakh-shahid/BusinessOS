...
export function buildOrderListWhere(
  ...
  if (assignedTo?.trim()) {
    ...
    const assigneeFilter: Prisma.OrderWhereInput = {
      OR: [
        { assignedToName: name },
    ...
    if (where.OR) {
      where.AND = [{ OR: where.OR }, assigneeFilter];
      delete where.OR;
    } else {
      Object.assign(where, assigneeFilter);
    }
  }
  ...
  if (search?.trim()) {
    ...
    where.OR = [
      ...
      { cuttingMasterName: { contains: q, mode: "insensitive" } },
      { stitchingMasterName: { contains: q, mode: "insensitive" } },
